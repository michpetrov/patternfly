...
(function (factory) {
   ...
   * @private
   */
  Pfly.pfFilter.init = function (coll) {
    var i;
    var ctx = coll.ctx;
    ...
    for (i = 0; i < ctx._pfFilter.filterCols.length; i++) {
      handleFilterOption(coll, i); // Need to pass value of i as a function
    }

    // Handle actions when enter is pressed within filter input
    handleFilterInput(coll);

    // Handle actions when clear filters control is selected
    handleClearFilters(coll);
  };

  // Local functions

  function applyFilters (coll) {
    var ctx = coll.ctx;

    if (ctx._pfFilter) {
      if (!ctx._pfFilter.filters || !ctx._pfFilter.filters.length) {
        $(coll.opts.rowSelector).show();
      }
      $(coll.opts.rowSelector).each(function (i, row) {
        var showRow = true;
        $.each(ctx._pfFilter.filters, function (index, filter) {
          if ($(row).find(coll.opts.columnSelectors[filter.column]).text().indexOf(filter.value) === -1) {
            showRow = false;
          }
        });
        $(row).toggle(showRow);
   ...
   * @param {string} filter.column - Column associated with Pfly
   * @param {string} filter.name - Name of the filter
   * @param {string} filter.value - Value of the filter
   * @private
   */
  function addActiveFilterControl (coll, filter) {
    var ctx = coll.ctx;
    var i;
   ...
   * @private
   */
  function addFilter (coll, filter) {
    var ctx = coll.ctx;
    var found = false;
    ...
    if (!found) {
      ...
      applyFilters(coll);
      addActiveFilterControl(coll, filter);
      updateFilterResults(coll);
    }
   ...
   * @private
   */
  function handleClearFilters (coll) {
    var ctx = coll.ctx;
   ...
   * @private
   */
  function handleFilterInput (coll) {
    var ctx = coll.ctx;
   ...
   * @private
   */
  function handleFilterOption (coll, i) {
    var ctx = coll.ctx;
   ...
   * @private
   */
  function updateFilterResults (coll) {
    var ctx = coll.ctx;
    var filteredRows = $(coll.opts.rowSelector + ":visible").length;
